# 13 Open-Source Projects

This chapter introduces robot hardware platforms — ROS/ROS2, robot arms, humanoid robots, sensors, and more.

## Contents

- [1. ROS / ROS2](13-01-ros.md)
  - Robot software frameworks
- [2. Robot Arms](13-02-robot-arms.md)
  - Franka, xArm, UR
- [3. Humanoid Robots](13-03-humanoid-robots.md)
  - Unitree, Atlas
- [4. End-Effectors & Sensors](13-04-end-effectors.md)
  - Grippers, cameras, force sensors

---

## Platform Comparison

| Type | Representative | Features |
|------|----------------|----------|
| Robot arm | Franka, UR | High precision, safe collaboration |
| Humanoid | Atlas, Unitree | Strong generality |
| Quadruped | Spot, Go1 | Terrain adaptability |
| Gripper | Robotiq, Weiss | Diverse grasping |

---

*This chapter is continuously updated...*
